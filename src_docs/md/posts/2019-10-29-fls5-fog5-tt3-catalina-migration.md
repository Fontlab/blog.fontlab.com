...
[Download macOS Mojave](https://apps.apple.com/us/app/macos-mojave/id1398502828) (6.1 GB installer)

**macOS 10.13.6 High Sierra** — requires at least 2 GB RAM and 14.3 GB storage in the VM.
[Download macOS High Sierra](https://apps.apple.com/us/app/macos-high-sierra/id1246284741) (4.8 GB installer)

**macOS 10.12.6 Sierra** — requires at least 2 GB RAM and 8.8 GB storage in the VM.
[Download macOS Sierra](https://itunes.apple.com/us/app/macos-sierra/id1127487414) (5.1 GB installer) — note: you can only download this from the App Store if you are currently on High Sierra or earlier.
...
4. In the *Installation images found* list, choose *macOS: Install macOS Mojave.app* and click *Continue*.
5. Click *Continue* in the popup and save the *macOS image file.dmg* in the suggested location (the *Parallels* folder inside your home folder). Wait for the image to be created.
6. In the *Name and Location* window, enter a name. Enable *Create alias on Mac desktop* and *Customize settings before installation*.
7. By default Parallels stores the VM in your *Parallels* folder. If you have both an SSD and an HDD, store it on the SSD.
8. Click *Create*. In the Configuration dialog:
   ...
   - *Hardware* tab → *CPU & Memory*: allocate 20–25% of your Mac's RAM (2 GB minimum; 4 GB if you have 16 GB)
   - *Graphics*: set Memory to 256 or 512 MB
   - Click *+ > Hard Disk*, set the maximum virtual disk size (32 GB if you allocated 4 GB RAM), enable *Split the disk image into 2 GB files*, click *OK*
...
10. Sign in to or create a Parallels Account to start the 14-day trial or activate your purchase.
11. The guest window appears. Click *▶* to start installing macOS Mojave.
12. If Parallels asks to use Accessibility features, click *Grant* and enable it in *System Preferences > Security & Privacy > Accessibility*.
13. When the *Welcome* screen appears in the guest, choose your language and click *→*. Press Ctrl+Alt to release mouse capture back to your real Mac.
14. In the *macOS Utilities* dialog inside the guest, double-click *Install macOS*.
15. Proceed through the installer; choose *Macintosh HD* (the virtual Hard Disk 2, not your real drive).
16. After 10–15 minutes the guest restarts. You don't need to sign in with an Apple ID unless you want to sync data with the guest.
17. Create a user account — same name as your real Mac account is fine, but use a different password.
18. Once setup completes, the guest desktop appears.
19. Click the yellow-triangle exclamation in the guest window's top-right corner and choose *Install Parallels Tools…*
20. Double-click the *Parallels Tools* volume on the guest desktop, double-click install, and complete the installation.
21. Enter the guest account's password when prompted (not your real Mac password).
22. Click *Restart*.
23. After restart, drag the *Parallels Tools* volume to the Bin inside the guest window. From the app menu, choose *Devices > CD/DVD > Disconnect*.
24. Double-click *Parallels Shared Folders* on the guest desktop. The guest Finder will show your real Mac's volumes — you can now open and save files across both systems.
25. On your real Mac, download the latest version of [FontLab Studio 5.1.6](https://www.fontlab.com/font-editor/fontlab-studio/), [Fontographer 5.2.4](https://www.fontlab.com/font-editor/fontographer/), or [TypeTool 3.1.3](https://www.fontlab.com/font-editor/typetool/).
26. Inside the guest, open *Parallels Shared Folders*, navigate to *Home > Downloads*, double-click the DMG, and install the font editor. Enter your serial number to activate.
...
29. In Parallels Desktop, choose *View > Expanded View*, click the gear icon, go to the *Hardware* tab, click *Hard Disk 1*. If the Source shows *macOS image file.hdd*, click *–* and choose *Move to Trash* — you no longer need the installer volume.
30. Quit Parallels Desktop.
31. Move *Install macOS Mojave.app* from your */Applications* folder to Trash.
32. Empty the Trash — you'll reclaim around 12 GB. Your guest macOS Mojave VM should now occupy no more than 24 GB.
