---
this_file: src_docs/md/posts/2013-09-22-opentype-layout-feature-classification.md
title: "OpenType Layout feature classification"
authors: [fontlab]
date:
  created: 2013-09-22
slug: opentype-layout-feature-classification
review:
  cta_status: ok
  cta_target: "https://help.fontlab.com/fontlab-vi/OpenType-features/"
  title_case: ok
  title_suggested: ""
  voice_quality: strong
  facts_to_verify:
    - "Classification was based on the OpenType spec version 1.6 — many tags have changed status since (e.g. some 'Removed' or 'Math' entries); flag as a 2013 snapshot"
    - "The 'before/during/after shaping' column reflects the author's own classification, not a normative spec field — verify framing"
    - "Microsoft-only MATH features (dtls, flac, ssty) are tied to the MATH table — verify against current spec"
  image_status: missing
  image_needs: "Optional: a small diagram of the GSUB/GPOS pipeline showing where shaping-driven features sit relative to user-facing typographic features."
  weakness_verdict: keep
  consolidate_with: ""
  notes: "Long reference table, valuable as a historical UI-design aid. Stripped all Microsoft otspec anchor URLs (link rot) down to plain tag code, dropped the empty Remarks column except where a note existed. Added a clear 2013-snapshot caveat up front since the OpenType registry has moved on considerably."
---
...
| `fina` | Terminal Forms | OpenType | special | special | none \| word, paragraph* | ARABIC, syrc \| ALL | during | shaping | Language | complex scripts |
...
| `init` | Initial Forms | OpenType | special | special | none \| word, paragraph* | ARABIC, syrc \| ALL | during | shaping | Language | complex scripts |
| `isol` | Isolated Forms | OpenType | special | special | none \| word, paragraph* | ARABIC, syrc \| ALL | during | shaping | Language | complex scripts |
...
| `medi` | Medial Forms | OpenType | special | special | none \| word, paragraph* | ARABIC, syrc \| ALL | during | shaping | Language | complex scripts |
...
[Read more →](https://help.fontlab.com/fontlab-vi/OpenType-features/){ .fl-help-cta }
