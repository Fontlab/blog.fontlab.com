...
2. **Compressed lowercase.** Italic letters are narrower than their roman counterparts — most easily seen on `n`, `o`, `e`.
3. **Cursive entry and exit strokes.** `i`, `n`, `m`, `u` start from the baseline with a small upstroke, end with a small upstroke into the next letter. These are the connection points cursive writing demands.
4. **Slant.** The slant follows from holding the pen at a steeper angle while writing quickly. It is the most visible feature and the least important — most of italic’s character comes from the structural changes above.
