---
this_file: src_docs/md/posts/2026-10-20-mingti-songti-minchotai-same-thing.md
...
In Taiwan and Hong Kong it is **Mingti** (明體).
In mainland China it is **Songti** (宋體).
In Japan it is **Minchōtai** (明朝体).
Three names implying descent from either the Ming or the Song dynasty.
...
Taiwan inherited *Mincho* via **Showa-era Japanese phototype imports**: Japanese vendors shipped film fonts under their own labels, and those labels never got removed.
...
For a FontLab user designing a CJK serif, the practical implication is that the *style* is settled across all three names, but the local conventions are not.
Preferred radical proportions, stroke contrast, where the brush memory shows — these draw differently in Tokyo, Taipei, and Beijing.
