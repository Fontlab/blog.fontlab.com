...
2. In the Font window, pick a glyph cell for your logo — "one" works, so the logo appears when you type the digit 1.
3. Open your EPS, PDF, or SVG in Illustrator or Affinity Designer, copy it, paste into the FontLab glyph cell. If prompted, choose "monochrome contours" and "Descender to Caps Height".
4. If any paths that should be black appear white, use *Contour > Join* to fix them.
5. Adjust sidebearings with the Metrics tool so the advance width matches the logo's intended space.
6. Export as OpenType PS from *Font > Export Font As*. Install the font, type "1", set the font to MyLogoFont in any application — your logo appears.
